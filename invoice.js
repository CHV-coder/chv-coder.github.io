...
Vue.filter('asDate', function(value) {
  ...
  return date.isValid() ? date.format('DD-MM-YY') : value;
});
...
function updateInvoice(row) {
  try {
    ...
    const importance = ['Date', 'Counterparty', 'Product', 'City', 'Number', 'Nubber2', 'Client', 'Items', 'Total', 'Invoicer', 'Due', 'Issued', 'Subtotal', 'Deduction', 'Taxes', 'Note'];
    ...
    if (!row.Subtotal && !row.Total && row.Items && Array.isArray(row.Items)) {
      try {
        ...
        row.Total = row.Subtotal - row.Items.Taxes;
      } catch (e) {
        console.error(e);
      }
    }
