...
Vue.filter('asDate2', function(value) {
  const date = moment.utc(value);
  return date.isValid() ? date.format('MMMM Do, YYYY') : value;
});
...
function updateInvoice(row) {
  try {
    ...
    const importance = ['Date', 'Number', 'Nubber2', 'Client', 'Items', 'Total', 'Invoicer', 'Due', 'Issued', 'Subtotal', 'Deduction', 'Taxes', 'Note'];
